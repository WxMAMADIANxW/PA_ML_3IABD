/*Fichier crée par El Walid IBRAHIM*/
#include "library.h"
#include <stdio.h>
#include <iostream>
#include <Eigen>
...
public:
    ...
    DLLEXPORT void forward_pass(vector<float> sample_inputs, bool is_classification) {
    ...
    DLLEXPORT void train_stochastic_gradient_backpropagation(vector<float> flattened_dataset_inputs,
        ...
        for (it = 0; it < iterations_count; it++) {
            ...
            vector<float> sample_inputs = reinterpret_cast<vector<float, allocator<float>> &&>(flattened_dataset_inputs[
                    k * input_dim,
                            (k + 1) * input_dim]);
...
DLLEXPORT void train_classification_stochastic_backprop_mlp_model(mlp MLP, vector<float> flattened_dataset_inputs,
                                                                  ...
                                                                  float alpha = 0.01,
                                                                  int iterations_count = 1000) {
    MLP.train_stochastic_gradient_backpropagation(flattened_dataset_inputs, flattened_expected_outputs, true, alpha,
                                                  iterations_count);
}
...
DLLEXPORT void train_regression_stochastic_backprop_mlp_model(mlp MLP, vector<float> flattened_dataset_inputs,
                                                              ...
                                                              float alpha = 0.01,
                                                              int iterations_count = 1000) {
    MLP.train_stochastic_gradient_backpropagation(flattened_dataset_inputs, flattened_expected_outputs, false, alpha,
                                                  iterations_count);
}
